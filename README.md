...
- **Текстовый чат** с историей диалога (SQLite, `data/bot_data.db`), не сбрасывается при рестарте. Сколько прошлых сообщений модель «помнит» при ответе, задаётся через `HISTORY_LIMIT` (по умолчанию 10; больше = лучше память, но медленнее ответы).
- **Резервные копии** — раз в сутки консистентный снимок базы складывается в `data/backups/`, хранятся последние 7 (настраивается через `BACKUP_INTERVAL_HOURS` и `BACKUP_KEEP`).
...
- **Документы** — пришлите PDF или текстовый файл (до 20 МБ) с вопросом в подписи, бот ответит по содержимому. Большие документы обрезаются до `DOC_MAX_CHARS` символов (по умолчанию 12000 — под небольшое контекстное окно локальных моделей).
- **Стриминг ответов** — ответ появляется в сообщении постепенно, по мере генерации (отключается через `STREAM_RESPONSES=false`).
- **Ограничение доступа** — впишите в `.env` `ALLOWED_USER_IDS=123,456` (Telegram ID через запятую), и бот будет отвечать только этим пользователям. Пустое значение = бот открыт всем. Чужому пользователю бот показывает его ID — так его легко добавить в список.
